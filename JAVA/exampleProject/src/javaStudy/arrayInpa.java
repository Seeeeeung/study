...
//	      });
	      ...
	      // 여러조건 비교
	      // 나이가 같은 사람이 있을 경우 추가적으로 이름순으로 정렬해야함
	      // comparing() 과 theComparing()을 이용해 체이닝하여 구현 가능
	      // 이때 getter / setter 를 이용해 속성을 가져와야함
	      class UserMore {
	    	  private String name;
	    	  private int age;
	    	  
	    	  UserMore(String name, int age) {
	    		  this.name = name;
	    		  this.age = age;
	    	  }
	      ...
	      UserMore[] usersMore = {
	              ...
	              new UserMore("김좌진", 20),
	              new UserMore("주몽", 50),
	              new UserMore("심사임당", 30)
	      };
	      
	      Arrays.sort(usersMore, Comparator.comparing(UserMore :: getAge)); // 나이순 정렬
	      for (UserMore u : usersMore) { System.out.println(u.name + " " + u.age + "세"); }// 출력
	      
	      System.out.println("\n");
	      
	      Arrays.sort(usersMore, Comparator.comparing(UserMore :: getName)); // 이름순 정렬
	      for (UserMore u : usersMore) { System.out.println(u.name + " " + u.age + "세"); }
	      
	      System.out.println("\n");
	      
	      Arrays.sort(usersMore, Comparator.comparing(UserMore :: getAge).thenComparing(UserMore :: getName)); // 나이순 먼저 정렬, 나이 같으면 따로 이름순 정렬
	      for(UserMore u : usersMore) { System.out.println(u.name + " " + u.age + "세"); }
